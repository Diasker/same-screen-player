...
应用会在用户数据目录的 `last-local-video.json` 中保存最近一次成功选择的本地视频绝对路径，所有分屏共用。下次选择文件时默认打开该文件所在目录；若文件已移动、删除或不可访问，则使用系统默认目录。取消选择不会覆盖记录。

网页主页面加载失败时，当前分屏会显示中文错误原因及诊断代码，并提供“重试”和“用 Chrome 打开”。该提示在发布版及两种操作模式下均可见；子框架或其他资源失败不会把整个分屏标为网页加载失败，Cloudflare 验证仍在原页面完成。
...
完成构建后，可运行 `node scripts/verify-load-errors.cjs` 验证分屏错误、重试、旧导航隔离、本地视频播放和跨重启目录记忆；检查使用独立临时配置与模拟文件选择器，不修改日常使用的设置。
